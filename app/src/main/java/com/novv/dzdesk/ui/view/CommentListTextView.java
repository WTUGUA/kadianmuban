...
public class CommentListTextView extends AppCompatTextView {
     ...
     * 人名称颜色
     */
    private int mNameColor = Color.parseColor("#ff05caee");
    private int mCommentColor = Color.parseColor("#8a000000");
    private int mTalkColor = Color.parseColor("#8a000000");
    ...
    public void setData(List<CommentModel> mInfos) {
        this.mInfos = mInfos;
        /**
    ...
    private SpannableStringBuilder getCommentString() {
        ...
        for (int mI = 0; mI < mInfos.size(); mI++) {
            final CommentModel mInfo = mInfos.get(mI);
            /**
             * 拼接成“张三：今天天气真好”这种形式，就是一行显示的文本。
             * 或者为张三 回复 李四：今天天气真好
             */
            String mS = null;
            if (mInfo.getCommentName() == null || mInfo.getCommentName().equals("")) {
                mS = mInfo.getReplyName() + "：" + mInfo.getReplyContent();
            } else {
                mS = mInfo.getReplyName() + mTalkStr + mInfo.getCommentName() + "：" + mInfo
                        .getReplyContent();
            }
            /**
             ...
             */
            int start = 0;
            int end = (mInfo.getReplyName()).length();
            ...
            mString.setSpan(new ForegroundColorSpan(mTalkColor), mInfo.getReplyName().length(),
                    mInfo.getReplyName().length() + mTalkStr.length(),
             ...
             */
            if (mInfo.getCommentName() != null && !mInfo.getCommentName().equals("")) {
                start = mInfo.getReplyName().length() + mTalkStr.length();
                end = mInfo.getReplyName().length() + mTalkStr.length() + mInfo.getCommentName()
                        .length();
                mString.setSpan(new ClickableSpan() {
                    @Override
                    public void updateDrawState(TextPaint ds) {
    ...
    public interface onCommentListener {
         ...
         * @param mInfo    评论相关信息
         */
        public void onNickNameClick(int position, CommentModel mInfo);
         ...
         * @param mInfo    评论相关信息
         */
        public void onToNickNameClick(int position, CommentModel mInfo);

        /**
         * 点击评论文本回调，主要针对对谁回复操作
         */
        public void onCommentItemClick(int position, CommentModel mInfo);
