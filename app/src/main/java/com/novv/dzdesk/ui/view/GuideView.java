 ...
 * -- show(添加GuideView进DecorView) -- GuideView.onMeasure -- GuideView.onLayout -- GuideView.Draw --
 * drawMaskLayer(绘制完毕)
 * <p>
 * view.post(new Runnable(){ public void run(){ new GuideView.Builder() .setTargetView()
 * .create().show(); } });
 * <p>
...
public class GuideView extends RelativeLayout {
    ...
    private void drawMaskLayer(Canvas canvas) {
        ...
        RectF rectF = new RectF(
                mTargetViewLocationX - mTransparentPaddingLeft + mTransparentMarginLeft,
                mTargetViewLocation[1] - mTransparentPaddingTop + mTransparentMarginTop,
                mTargetViewLocationX + mTargetViewWidth + mTransparentPaddingRight
                        - mTransparentMarginRight,
                mTargetViewLocation[1] + mTargetViewHeight + mTransparentPaddingBottom
                        - mTransparentMarginBottom);

        switch (mFormType) {
            case Form.CIRCLE:
                mTemp.drawCircle(mTargetViewLocationX + mTargetViewWidth / 2,
                        mTargetViewLocation[1] + mTargetViewWidth / 2, mTargetViewWidth / 2,
                        mTransparentPaint);
                break;
            case Form.ELLIPSE:
            ...
            case Form.REACTANGLE:
                mTemp.drawRect(mTargetViewLocationX, mTargetViewLocation[1],
                        mTargetViewLocationX + mTargetViewWidth,
                        mTargetViewLocation[1] + mTargetViewHeight,
                        mTransparentPaint);
                break;
            case Form.CIRCLE_LONG:
                int longRadius = Math.max(mTargetViewWidth, mTargetViewHeight);
                mTemp.drawCircle(mTargetViewLocation[0] + mTargetViewWidth / 2,
                        mTargetViewLocation[1] + mTargetViewWidth / 2, longRadius / 2,
                        mTransparentPaint);
                break;
            case Form.CIRCLE_SHORT:
                int shortRadius = Math.min(mTargetViewWidth, mTargetViewHeight);
                mTemp.drawCircle(mTargetViewLocation[0] + mTargetViewWidth / 2,
                        mTargetViewLocation[1] + mTargetViewWidth / 2, shortRadius / 2,
                        mTransparentPaint);
                break;
        }
        //绘制到GuideView的画布上
    ...
    private void addHintView() {
        ...
        if (mHintView != null) {
            ...
            switch (mHintViewDirection) {
                ...
                case Direction.LEFT:
                    ...
                    layoutParams.setMargins(0, mTargetViewLocation[1],
                            mScreenWidth - mTargetViewLocation[0] + mHintViewSpace
                                    + mHintViewMarginRight, 0);
                    break;
                case Direction.LEFT_BOTTOM:
                    this.setGravity(Gravity.RIGHT | Gravity.TOP);
                    layoutParams.setMargins(0,
                            mTargetViewLocation[1] + mTargetViewHeight + mHintViewSpace
                                    + mHintViewMarginTop,
                            mScreenWidth - mTargetViewLocation[0] + mHintViewSpace
                                    + mHintViewMarginRight, 0);
                    break;
                case Direction.LEFT_ABOVE:
                    ...
                    layoutParams.setMargins(0, 0,
                            mScreenWidth - mTargetViewLocation[0] + mHintViewSpace
                                    + mHintViewMarginRight,
                            mScreenHeight - mTargetViewLocation[1] + mHintViewSpace
                                    + mHintViewMarginBottom);
                    break;
                ...
                case Direction.RIGHT:
                    ...
                    layoutParams.setMargins(
                            mTargetViewLocation[0] + mTargetViewWidth + mHintViewSpace
                                    + mHintViewMarginLeft,
                ...
                case Direction.RIGHT_ABOVE:
                    ...
                    layoutParams.setMargins(
                            mTargetViewWidth + mTargetViewLocation[0] + mHintViewSpace
                                    + mHintViewMarginLeft, 0,
                            0, mScreenHeight - mTargetViewLocation[1] + mHintViewSpace
                                    + mHintViewMarginBottom);
                    break;
                ...
                case Direction.ABOVE_ALIGN_RIGHT:
                    ...
                    layoutParams.setMargins(0, 0,
                            mScreenWidth - mTargetViewLocation[0] - mTargetViewWidth
                                    + mHintViewMarginRight,
                            mScreenHeight - mTargetViewLocation[1] + mHintViewSpace
                                    + mHintViewMarginBottom);
                    break;
                ...
                case Direction.BOTTOM:
                    ...
                    layoutParams
                            .setMargins(0,
                                    mTargetViewLocation[1] + mTargetViewHeight + mHintViewMarginTop,
                                    0, 0);
                    break;
            ...
            ViewGroup viewGroup = (ViewGroup) mHintView.getParent();
            if (viewGroup != null) {
    ...
    public void getTargetViewPosition() {
        ...
        if (mTargetView.getWidth() > 0 && mTargetView.getHeight() > 0) {
            ...
            Log.i(TAG, "targetView.width : " + mTargetView.getWidth() + " location x : "
                    + mTargetViewLocation[0]);
            Log.i(TAG, "targetView.height : " + mTargetView.getHeight() + " location y : "
                    + mTargetViewLocation[1]);
        } else {
            hasMeasure = false;
            Log.i(TAG,
                    "targetView is not measured,please user view.post(Runnable run) initialize GuideView");
        }
    }
