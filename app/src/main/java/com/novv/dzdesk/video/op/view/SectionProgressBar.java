    ...
    protected void onDraw(Canvas canvas) {
        ...
        synchronized (this) {
            if (!mBreakPointInfoList.isEmpty()) {
                ...
                for (BreakPointInfo info : mBreakPointInfoList) {
                    ...
                    canvas
                            .drawRect(startPoint, 0, startPoint + DEFAULT_BREAK_POINT_WIDTH,
                                    getMeasuredHeight(),
                                    mBreakPointPaint);
            ...
            if (mBreakPointInfoList.isEmpty()
                ...
                canvas.drawRect(mPixelUnit * mFirstPointTime, 0,
                        mPixelUnit * mFirstPointTime + DEFAULT_BREAK_POINT_WIDTH,
                        getMeasuredHeight(),
                        mFirstPointPaint);
            }
        }
        ...
        if (mCurrentState == State.START) {
            ...
            if (startPoint + mProgressWidth <= getMeasuredWidth()) {
                canvas.drawRect(startPoint, 0, startPoint + mProgressWidth, getMeasuredHeight(),
                        mProgressBarPaint);
            } else {
                canvas.drawRect(startPoint, 0, getMeasuredWidth(), getMeasuredHeight(),
                        mProgressBarPaint);
            }
        }
        ...
        if (mIsCursorVisible) {
            if (mCurrentState == State.START) {
                canvas.drawRect(startPoint + mProgressWidth, 0,
                        startPoint + DEFAULT_CURSOR_WIDTH + mProgressWidth, getMeasuredHeight(),
                        mCursorPaint);
            } else {
                canvas.drawRect(startPoint, 0, startPoint + DEFAULT_CURSOR_WIDTH,
                        getMeasuredHeight(),
                        mCursorPaint);
            }
        }
